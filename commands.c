...
// extern tells the compiler that the variable is saved somewhere else
...
static char* prev_path = NULL; //saves the previous directory path
...
int ExeCmd(PJOB *jobs, char* lineSize, char* cmdString)
{
	...
	strcpy(history[hist_counter%50],lineSize);
	hist_counter++;
